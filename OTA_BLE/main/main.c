...
// UUIDs (must be variables, not macros)
                                ...
                                esp_ble_gatts_cb_param_t *param)
{
    switch(event){
        ...
        case ESP_GATTS_REG_EVT:
            ...
            ESP_LOGI("GATTS", "GATT Registered");
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT:
            ...
            if (param->add_attr_tab.status == ESP_GATT_OK) {
                ...
                ESP_LOGI("GATTS", "Service started");

                esp_ble_gatts_start_service(
                    param->add_attr_tab.handles[IDX_SVC]
                );
            }
            break;

        case ESP_GATTS_READ_EVT:
            ESP_LOGI("GATTS", "Read: %d", counter);
            break;

        case ESP_GATTS_WRITE_EVT:
            counter = param->write.value[0];
            ESP_LOGI("GATTS", "Write: %d", counter);
            break;
